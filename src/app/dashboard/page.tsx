    ...
    <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-5">
        ...
        <div className="rounded-xl border bg-white p-5">
          ...
          <p className="mt-2 text-lg font-bold text-emerald-600">{host?.google_refresh_token ? "Connected" : "Not connected"}</p>
        </div>
      </div>
      <div>
        ...
        {!upcomingBookings?.length ? (
          ...
          <div className="space-y-3">
            {upcomingBookings.map((b) => {
              ...
              return (
                <div key={b.id} className="flex items-center justify-between rounded-xl border bg-white p-4 gap-3">
                  <div className="flex items-center gap-4">
                    <div className="flex flex-col items-center rounded-lg bg-blue-50 px-3.5 py-2 text-center">
                      <span className="text-[10px] font-semibold uppercase text-blue-500">{start.toLocaleDateString("en-US", { month: "short" })}</span>
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-sm text-gray-500">with {b.guest_name} &middot; {start.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</p>
                    </div>
                  </div>
                  <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">{b.status}</span>
                </div>
              );
            })}
          </div>
