...
export async function POST(request: NextRequest) {
  ...
  const videoProvider = meetingType.video_provider || "google_meet";

  let googleEventId = null;
  let videoLink: string | null = null;

  // Google Meet: create calendar event WITH conferencing
  if (videoProvider === "google_meet" && host.google_refresh_token) {
    try {
  ...
  // Teams: create calendar event WITHOUT conferencing, attach Teams link
  if (videoProvider === "teams" && host.google_refresh_token) {
    const teamsUrl = host.teams_link || null;
    videoLink = teamsUrl;

    try {
      const accessToken = await refreshAccessToken(host.google_refresh_token);
      // Create calendar event with Teams link in description (no Google Meet)
      const calEvent = await createCalendarEvent({
        accessToken,
        summary: `${meetingType.title} - ${guest_name}`,
        description: `Booked via Apploi Scheduling\n\nGuest: ${guest_name}\nEmail: ${guest_email}${teamsUrl ? `\nMicrosoft Teams: ${teamsUrl}` : ""}${guest_notes ? `\nNotes: ${guest_notes}` : ""}`,
        startTime: startsAt.toISOString(),
        endTime: endsAt.toISOString(),
        attendeeEmail: guest_email,
        attendeeName: guest_name,
  ...
  // No video: create calendar event WITHOUT conferencing
  if (videoProvider === "none" && host.google_refresh_token) {
    try {
      const accessToken = await refreshAccessToken(host.google_refresh_token);
      const calEvent = await createCalendarEvent({
        accessToken,
  ...
  // Build video join text for email
  const videoJoinHtml = videoLink
    ? videoProvider === "teams"
      ? `<p style="margin: 0; font-size: 14px;"><strong>Join:</strong> <a href="${videoLink}" style="color: #00A1AB;">Join Microsoft Teams Meeting</a></p>`
      : `<p style="margin: 0; font-size: 14px;"><strong>Join:</strong> <a href="${videoLink}" style="color: #00A1AB;">${videoLink}</a></p>`
    : "";
  ...
  logAudit({
    ...
    details: { guest_email, meeting_type: meetingType.title, video_provider: videoProvider, calendar_synced: !!googleEventId },
    ipAddress: clientIp,
